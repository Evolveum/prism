...
public class PrismQueryLanguageParserImpl implements PrismQueryLanguageParser {
    ...
    public static class FilterArgumentSpec<T> {
        ...
        public FilterArgumentSpec(QName name, Class<T> type, T defaultValue) {super();
            this.name = name;
            this.type = type;
            this.required = false;
    ...
    private final ItemFilterFactory equalFilter = new PropertyFilterFactory() {
        ...
        public ObjectFilter create(QueryParsingContext.Local context, ItemPath path,
             ...
             */
            if (definition instanceof PrismReferenceDefinition refDef) {
    ...
    private final Map<QName, ItemFilterFactory> filterFactories = ImmutableMap.<QName, ItemFilterFactory>builder()
            .put(EQUAL.getName(), equalFilter)
            .put(ANY_IN.getName(), new PropertyFilterFactory() {
            ...
            .put(NOT_EQUAL.getName(), new ItemFilterFactory() {
            ...
            .put(GREATER.getName(), new PropertyFilterFactory() {
                @Override
            ...
            .put(GREATER_OR_EQUAL.getName(), new PropertyFilterFactory() {
                @Override
                ObjectFilter valueFilter(PrismPropertyDefinition<?> definition, ItemPath path, QName matchingRule,
                        Object value) {
            ...
            .put(LESS.getName(), new PropertyFilterFactory() {
                @Override
                ObjectFilter valueFilter(PrismPropertyDefinition<?> definition, ItemPath path, QName matchingRule,
                        Object value) {
            ...
            .put(LESS_OR_EQUAL.getName(), new PropertyFilterFactory() {
                @Override
                ObjectFilter valueFilter(PrismPropertyDefinition<?> definition, ItemPath path, QName matchingRule,
                        Object value) {
            ...
            .put(CONTAINS.getName(), new SubstringFilterFactory(false, false))
            .put(STARTS_WITH.getName(), new SubstringFilterFactory(true, false))
            .put(ENDS_WITH.getName(), new SubstringFilterFactory(false, true))
            .put(MATCHES.getName(), this::matchesFilter)
            .put(EXISTS.getName(), new ItemFilterFactory() {
                @Override
                public ObjectFilter create(QueryParsingContext.Local context, ItemPath itemPath,
                    ...
                    if (itemDef instanceof PrismPropertyDefinition<?> definition) {
                        return NotFilterImpl.createNot(EqualFilterImpl.createEqual(itemPath, definition, matchingRule));
                    }
                    return ExistsFilterImpl.createExists(itemPath, context.itemDef(), null);
                }
            })
            .put(FULL_TEXT.getName(), new SelfFilterFactory("fullText") {
            ...
            .put(IN_OID.getName(), new SelfFilterFactory("inOid") {
            ...
            .put(OWNED_BY_OID.getName(), new SelfFilterFactory("ownedByOid") {
            ...
            .put(IN_ORG.getName(), new SelfFilterFactory("inOrg") {
            ...
            .put(IS_ROOT.getName(), new SelfFilterFactory("isRoot") {
            ...
            .put(TYPE.getName(), new SelfFilterFactory("type") {
            ...
            .put(REFERENCED_BY.getName(), new SelfFilterFactory(REFERENCED_BY.getName()) {
            ...
            .put(OWNED_BY.getName(), new SelfFilterFactory(OWNED_BY.getName()) {
            ...
            .put(LEVENSHTEIN.getName(), new FunctionLikeFilterFactory(LEVENSHTEIN.getName(), LEVENSHTEIN_ARGUMENTS) {
            ...
            .put(SIMILARITY.getName(), new FunctionLikeFilterFactory(SIMILARITY.getName(), SIMILARITY_ARGUMENTS) {
    ...
    private final Map<QName, ItemFilterFactory> notFilterFactories = ImmutableMap.<QName, ItemFilterFactory>builder()
            .put(EXISTS.getName(), new ItemFilterFactory() {
                ...
                public ObjectFilter create(QueryParsingContext.Local context, ItemPath itemPath,
                        ItemDefinition<?> itemDef, QName matchingRule, SubfilterOrValueContext subfilterOrValue) {
                    if (itemDef instanceof PrismPropertyDefinition<?> definition) {
                        return EqualFilterImpl.createEqual(itemPath, definition,
                                matchingRule);
                    }
    ...
    protected PreparedPrismQuery parse(ItemDefinition<?> contextDef, AxiomQuerySource source, boolean placeholdersEnabled) throws SchemaException {
        ComplexTypeDefinition typeDef = contextDef instanceof PrismContainerDefinition<?> pcd
                ? pcd.getComplexTypeDefinition()
                : null;
    ...
    private ObjectFilter parseFilter(
        ...
        } else if (root instanceof GenFilterContext rootFilter) {
            return itemFilter(context, rootFilter.itemFilter());
        } else if (root instanceof SubFilterContext rootFilter) {
            return parseFilter(context, rootFilter.subfilterSpec().filter());
        }
    ...
    private class TypeOverride extends FilterBasedOverride<TypeFilter> {
        ...
        public boolean isApplicable(QueryParsingContext.Local context, ItemFilterContext itemFilter) {
            ...
            return itemFilter.negation() == null && Filter.Name.TYPE.getName().equals(filterName(itemFilter));
        }
    ...
    private class OwnedByOverride extends FilterBasedOverride<OwnedByFilter> {
        ...
        public boolean isApplicable(QueryParsingContext.Local context, ItemFilterContext itemFilter) {
            ...
            return OWNED_BY.getName().equals(filterName(itemFilter));
        }
    ...
    private ObjectFilter andFilter(QueryParsingContext.Local context, List<FilterContext> unparsed) throws SchemaException {
        ...
        while (iterator.hasNext()) {
            ...
            if (next instanceof GenFilterContext filterContext) {
                ItemFilterContext itemFilter = filterContext.itemFilter();
    ...
    private <E extends FilterContext> void expand(List<FilterContext> expanded,
        ...
        for (FilterContext filterContext : notExpanded) {
            if (filterContext instanceof SubFilterContext subfilter) {
    ...
    QName filterName(ItemFilterContext filter) {
        if (filter.filterNameAlias() != null) {
            return Filter.fromAlias(filter.filterNameAlias().getText()).orElseThrow();
        }
        FilterNameContext filterName = filter.filterName();
        if (filterName.filterNameAlias() != null) {
            return Filter.fromAlias(filterName.filterNameAlias().getText()).orElseThrow();
        }
    ...
    private String extractTextForm(LiteralValueContext string) {
        if (string instanceof StringValueContext valueContext) {
            return AxiomAntlrLiterals.convertString(valueContext);
        }
    ...
    private ObjectFilter matchesFilter(
        ...
        if (definition instanceof PrismContainerDefinition<?> containerDef) {
            ...
            return matchesReferenceFilter(context, path, referenceDefinition,
    ...
    private Map<String, Object> valuesFromFilter(String typeName, Map<String, Class<?>> props, FilterContext child,
        ...
        if (child instanceof GenFilterContext filterContext) {
            ItemFilterContext filter = filterContext.itemFilter();
            if (EQUAL.getName().equals(filterName(filter))) {
        ...
        } else if (child instanceof AndFilterContext filterContext) {
            valuesFromFilter(typeName, props, filterContext.left, result, strict);
            valuesFromFilter(typeName, props, filterContext.right, result, strict);
        } else if (strict) {
            throw new SchemaException("Only 'equals' and 'and' filters are supported.");
        }
        return result;
    ...
    private ItemFilterContext consumeFromAnd(String path, QName filterName, Collection<FilterContext> andFilters) {
        ...
        while (iterator.hasNext()) {
            ...
            if (maybe instanceof GenFilterContext filterContext) {
                ItemFilterContext filter = filterContext.itemFilter();
    ...
    private boolean andContains(String path, Collection<FilterContext> andFilters) {
        for (FilterContext maybe : andFilters) {
            if (maybe instanceof GenFilterContext filterContext) {
                ...
                // If we have equals filter and name matches, extract value and remove it from list
                // for further processing
                if (EQUAL.getName().equals(filterName(filter)) && path.equals(filter.path().getText())) {
                    return true;
                }
            }
        }
